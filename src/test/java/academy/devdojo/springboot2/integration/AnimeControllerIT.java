...
class AnimeControllerIT {
    ...
    private static final DevDojoUser USER = DevDojoUser.builder()
            ...
            .password("{bcrypt}$2a$10$3JZxSMrmuYiA9T.qPloEXugmZZStxtY0RVzB75utGy6J1B4LzH1vC")
            .username("devdojo")
    ...
    static class Config {
        ...
        public TestRestTemplate testRestTemplateRoleAdminCreator(@Value("${local.server.port}") int port) {
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    .rootUri("http://localhost:" + port)
                    .basicAuthentication("renato", "academy");
            return new TestRestTemplate(restTemplateBuilder);
        }
    }
    ...
    @Test
    @DisplayName("findByName returns an empty when anime is not found")
    void findByName_ReturnsEmptyListOfAnime_WhenAnimeIsNotFound() {
        devDojoUserRepository.save(USER);
        String notExistentAnime = "troll";
        String url = String.format("/animes/find?name=%s", notExistentAnime);
        List<Anime> animes = testRestTemplateRoleUser.exchange(url, HttpMethod.GET, null,
                new ParameterizedTypeReference<List<Anime>>() {
                }).getBody();
        Assertions.assertThat(animes)
